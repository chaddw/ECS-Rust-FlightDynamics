...
fn calc_airplane_loads(rigidbod: &mut RigidBody)
{
    ...
    for i in 0..8 
    {
        ...
        {
            v_resultant = common::Myvec::multiplyscalar(&v_drag_vector, 0.5 * tmp); //simulate fuselage drag
...
impl<'a> System<'a> for MakePacket
{
    ...
    fn run(&mut self, (rigidbody, mut packet): Self::SystemData) 
    {
        for (rigidbod, mut pckt) in (&rigidbody, &mut packet).join() 
        {
            ...
            //Set lat, long, alt
            fdm.latitude = f64::from_be_bytes((rigidbod.v_position.x as f64).to_ne_bytes());
            fdm.longitude = f64::from_be_bytes((rigidbod.v_position.y as f64).to_ne_bytes()); 
            fdm.altitude = f64::from_be_bytes((rigidbod.v_position.z as f64).to_ne_bytes()); //flightgear wants meters here, but on screen it displays feet
